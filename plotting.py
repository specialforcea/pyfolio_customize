...
def plot_annual_returns(returns, ax=None, **kwargs):
    ...
    **kwargs, optional
    ...
    ax.axvline(
        ...
        lw=4,
        alpha=0.7)
    sorted_ann_return = 100 * ann_ret_df.sort_index(ascending=False)
    # (100 * ann_ret_df.sort_index(ascending=False)
    #  ).plot(ax=ax, kind='barh', alpha=0.70, **kwargs)
    y_pos = np.arange(len(sorted_ann_return))
    ax.barh(y_pos, sorted_ann_return.values[:,0])
    ax.set_yticks(y_pos)
    ax.set_yticklabels(sorted_ann_return.index)
...
def plot_drawdown_periods(returns, top=10, ax=None, **kwargs):
    ...
    **kwargs, optional
    ...
    ax.plot(df_cum_rets.index, df_cum_rets.values)
    lim = ax.get_ylim()
...
def plot_drawdown_underwater(returns, ax=None, **kwargs):
    ...
    **kwargs, optional
    ...
    ax.fill_between(underwater.index, underwater.values,0,color='coral')
...
STAT_FUNCS_PCT = [
    'Annual return',
    'Annualized_1year_return',
    'Annualized_5year_return',
    'Annualized_10year_return',
    'Cumulative returns',
    'Annual volatility',
    'Annuallized_1year_volatility',
    'Annuallized_5year_volatility',
    'Annuallized_10year_volatility',
    'Max drawdown',
    'Daily value at risk',
    'Daily turnover'
]
...
def plot_returns(returns,
    ...
    **kwargs, optional
    ...
    if live_start_date is not None:
        ...
        ax.plot(returns.index, returns.values, color='green')
    return ax
...
def plot_rolling_returns(returns,
    ...
    **kwargs, optional
    ...
    if factor_returns is not None:
        ...
        ax.plot(cum_factor_returns.index, cum_factor_returns.values,
                label=factor_returns.name, color='gray', linewidth=3)
    ...
    ax.plot(is_cum_returns.index, is_cum_returns.values, label=is_cum_returns.name,
...
def plot_rolling_beta(returns, factor_returns, legend_loc='best',
    ...
    **kwargs, optional
    ...
    ax.plot(rb_1.index, rb_1.values, color='steelblue', linewidth=3)
    ...
    ax.plot(rb_2.index, rb_2.values, color='gray', linewidth=3)
...
def plot_rolling_volatility(returns, factor_returns=None,
    ...
    **kwargs, optional
    ...
    ax.plot(rolling_vol_ts.index, rolling_vol_ts.values, color='orangered', linewidth=3)
    ...
    if factor_returns is not None:
        ...
        ax.plot(rolling_vol_ts_factor.index, rolling_vol_ts_factor.values, color='grey')
...
def plot_rolling_sharpe(returns, factor_returns=None,
    ...
    **kwargs, optional
    ...
    ax.plot(rolling_sharpe_ts.index, rolling_sharpe_ts.values, color='orangered')
    if factor_returns is not None:
        rolling_sharpe_ts_factor = timeseries.rolling_sharpe(
            factor_returns, rolling_window)
        # rolling_sharpe_ts_factor.plot(alpha=.7, lw=3, color='grey', ax=ax,
        #                               **kwargs)
        ax.plot(rolling_sharpe_ts_factor.index, rolling_sharpe_ts_factor.values, color='grey')
