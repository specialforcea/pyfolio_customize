...
@plotting.customize
def compute_stats(returns, positions=None,
                              transactions=None,
                              live_start_date=None,
                              cone_std=(1.0, 1.5, 2.0),
                              benchmark_rets=None,
    ...
    for col in returns.columns:
        ...
        perf_stats = plotting.show_perf_stats(ret, benchmark_rets,
                                              positions=positions,
                                              transactions=transactions,
                                              turnover_denom=turnover_denom,
                                              bootstrap=bootstrap,
                                              live_start_date=live_start_date,
                                              header_rows=header_rows)
        perf_stats = perf_stats.rename(columns={'Backtest':col})
        try:
            df_stats = df_stats.join(perf_stats)
        except:
            df_stats = perf_stats
